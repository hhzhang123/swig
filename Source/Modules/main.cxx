...
/* usage string split into multiple parts otherwise string is too big for some
 * compilers */
/* naming conventions for commandline options - no underscores, no capital
 * letters, join words together
 * except when using a common prefix, then use '-' to separate, eg the debug-xxx
 * options */
...
static bool check_extension(String *filename) {
    ...
    if ((strcmp(c, ".c") == 0) || (strcmp(c, ".C") == 0) ||
        (strcmp(c, ".cc") == 0) || (strcmp(c, ".cxx") == 0) ||
        (strcmp(c, ".c++") == 0) || (strcmp(c, ".cpp") == 0)) {
        wanted = true;
    }
    Delete(extension);
    return wanted;
...
static void configure_outdir(const String *c_wrapper_outfile) {
    // Use the C wrapper file's directory if the output directory has not been
    // set by user
    ...
    if (Len(outdir) > 0) {
        ...
        if (strcmp(outd + strlen(outd) - strlen(SWIG_FILE_DELIMITER),
                   SWIG_FILE_DELIMITER) != 0)
...
// Returns the directory for generating language specific files (non C/C++
// files)
...
void SWIG_config_cppext(const char *ext) { cpp_extension = ext; }
        ...
        if (argv[i] && !Swig_check_marked(i)) {
            if (strncmp(argv[i], "-I-", 3) == 0) {
                ...
                Swig_mark_arg(i);
                       ...
                       0) {  // Used to also accept -c. removed in swig-1.3.36
                Swig_mark_arg(i);
                Swig_warning(WARN_DEPRECATED_OPTC, "SWIG", 1,
                             "-runtime, -noruntime command line options are "
                             "deprecated.\n");
                SwigRuntime = 1;
                ...
                if (argv[i + 1]) {
                    ...
                    i++;
                       ...
                       (strcmp(argv[i], "-makedefault") == 0)) {
                GenerateDefault = 1;
                Swig_mark_arg(i);
            } else if ((strcmp(argv[i], "-no_default") == 0) ||
                       (strcmp(argv[i], "-nodefault") == 0)) {
                GenerateDefault = 0;
                Swig_warning(
                    WARN_DEPRECATED_NODEFAULT, "SWIG", 1,
                ...
                Swig_warning(WARN_DEPRECATED_NOEXTERN, "SWIG", 1,
                             "-noextern command line option is deprecated; "
                             "extern is no longer generated by default.\n");
                AddExtern = 0;
                Swig_mark_arg(i);
                ...
                Swig_mark_arg(i);
                       ...
                       (strcmp(argv[i], "-debug_template") == 0) ||
                       (strcmp(argv[i], "-show_templates") == 0)) {
                ...
                Printf(stdout, "%s\n", SwigLib);
                if (SwigLibWinUnix) Printf(stdout, "%s\n", SwigLibWinUnix);
                SWIG_exit(EXIT_SUCCESS);
                ...
                if (argv[i + 1]) {
                    ...
                    if (!outfile_name_h || !dependencies_file) {
                        ...
                        String *basename =
                            ext ? NewStringWithSize(
                                      Char(outfile_name),
                                      (int)(Char(ext) - Char(outfile_name)))
                                : NewString(outfile_name);
                ...
                fprintf(stdout,
                        "\nPlease see %s for reporting bugs and further "
                        "information\n",
                        PACKAGE_BUGREPORT);
                SWIG_exit(EXIT_SUCCESS);
                ...
                if (argv[i + 1]) {
                    ...
                    Swig_arg_error();
                       ...
                       (strcmp(argv[i], "-debug_typemap") == 0) ||
                       (strcmp(argv[i], "-tm_debug") == 0)) {
                tm_debug = 1;
                Swig_mark_arg(i);
                ...
                Swig_mark_arg(i);
                       ...
                       (strcmp(argv[i], "-dump_typedef") == 0)) {
                dump_typedef = 1;
                Swig_mark_arg(i);
            } else if ((strcmp(argv[i], "-debug-classes") == 0) ||
                       (strcmp(argv[i], "-dump_classes") == 0)) {
                dump_classes = 1;
                Swig_mark_arg(i);
            } else if ((strcmp(argv[i], "-debug-memory") == 0) ||
                       (strcmp(argv[i], "-dump_memory") == 0)) {
                memory_debug = 1;
                Swig_mark_arg(i);
...
int SWIG_main(int argc, char *argv[], const TargetLanguageModule *tlm) {
    ...
    // Set the SWIG version value in format 0xAABBCC from package version
    // expected to be in format A.B.C
    String *package_version =
        NewString(PACKAGE_VERSION); /* Note that the fakeversion has not been
                                       set at this point */
    ...
    // Inform the parser if the nested classes should be ignored unless
    // explicitly told otherwise via feature:flatnested
    ignore_nested_classes =
        lang->nestedClassesSupport() == Language::NCS_Unknown ? 1 : 0;
    ...
    if ((c = getenv("SWIG_LIB")) == (char *)0) {
        ...
        if (!(GetModuleFileName(0, buf, MAX_PATH) == 0 ||
            ...
            *(p + 1) = '\0';
            SwigLib =
                NewStringf("%sLib", buf);  // Native windows installation path
        } else {
            SwigLib = NewStringf("");  // Unexpected error
        }
        if (Len(SWIG_LIB_WIN_UNIX) > 0)
            SwigLibWinUnix =
                NewString(SWIG_LIB_WIN_UNIX);  // Unix installation path using a
                                               // drive letter (for msys/mingw)
#else
    ...
    if (CPlusPlus) Preprocessor_define((DOH *)"__cplusplus __cplusplus", 0);
    ...
    if (help) {
        Printf(stdout,
               "\nNote: 'swig -<lang> -help' displays options for a specific "
               "target language.\n\n");
    ...
    if (CPlusPlus && cparse_cplusplusout) {
        Printf(stderr,
               "The -c++out option is for C input but C++ input has been "
               "requested via -c++\n");
        SWIG_exit(EXIT_FAILURE);
    }
    ...
    {
        String *rl = NewString("");
        Printf(rl, ".%sswig_lib%s%s", SWIG_FILE_DELIMITER, SWIG_FILE_DELIMITER,
               LangSubDir);
        Swig_add_directory(rl);
        if (SwigLibWinUnix) {
            rl = NewString("");
            Printf(rl, "%s%s%s", SwigLibWinUnix, SWIG_FILE_DELIMITER,
                   LangSubDir);
            Swig_add_directory(rl);
        }
        rl = NewString("");
    ...
    if (SwigLibWinUnix) Swig_add_directory((String *)SwigLibWinUnix);
    ...
    if (external_runtime) SWIG_dump_runtime();
    ...
    if (checkout) {
        ...
        if (outfile_name) outfile = outfile_name;

        if (Verbose) Printf(stdout, "Handling checkout...\n");
        ...
        if (Verbose) Printf(stdout, "Preprocessing...\n");

        {
            ...
            if (!df) {
                ...
                if (!df) {
                    ...
                    if (cfile && cfile[0] == '-') {
                        Printf(stderr, "Unable to find option or file '%s', ",
                               input_file);
                        Printf(stderr,
                    ...
                    Swig_warning(WARN_DEPRECATED_INPUT_FILE, "SWIG", 1,
                                 "Use of the include path to find the input "
                                 "file is deprecated and will not work with "
                                 "ccache. Please include the path when "
            ...
            if (!no_cpp) {
                ...
                Printf(fs, "%%include(maininput=\"%s\") \"%s\"\n",
                       Swig_filename_escape(input_file),
            ...
            if (depend) {
                if (!no_cpp) {
                    ...
                    String *inputfile_filename =
                        outcurrentdir ? Swig_file_filename(input_file)
                                      : Copy(input_file);
                    String *basename = Swig_file_basename(inputfile_filename);
                    if (!outfile_name) {
                        if (CPlusPlus || lang->cplus_runtime_mode()) {
                            outfile = NewStringf("%s_wrap.%s", basename,
                                                 cpp_extension);
                        } else {
                    ...
                    if (dependencies_file && Len(dependencies_file) != 0) {
                        ...
                        String *filename = NewStringf("%s_wrap.%s", basename,
                                                      depends_extension);
                        f_dependencies_file =
                            NewFile(filename, "w", SWIG_output_files());
                        if (!f_dependencies_file) {
                    ...
                    for (int i = 0; i < Len(files); i++) {
                        int use_file = 1;
                        if (depend == 2) {
                            if ((Strncmp(Getitem(files, i), SwigLib,
                                         Len(SwigLib)) == 0) ||
                                (SwigLibWinUnix &&
                                 (Strncmp(Getitem(files, i), SwigLibWinUnix,
                                          Len(SwigLibWinUnix)) == 0)))
                                use_file = 0;
                        }
                        if (use_file) {
                            Printf(f_dependencies_file, "\\\n  %s ",
                    ...
                    if (depend_only) SWIG_exit(EXIT_SUCCESS);
                    ...
                    // Actually we could but it would be inefficient when just
                    // generating dependencies, as it would be done after
                    // Swig_cparse
                    SWIG_exit(EXIT_FAILURE);
                }
            }
        ...
        if (!CPlusPlus) {
            if (Verbose) Printf(stdout, "Processing unnamed structs...\n");
        ...
        if (CPlusPlus) {
            if (Verbose) Printf(stdout, "Processing nested classes...\n");
        ...
        if (top) {
            if (!Getattr(top, "name")) {
                ...
                Setattr(top, "infile",
                        infile);  // Note: if nopreprocess then infile is the
                                  // original input file, otherwise input_file
                Setattr(top, "inputfile", input_file);

                String *infile_filename =
                    outcurrentdir ? Swig_file_filename(infile) : Copy(infile);
                String *basename = Swig_file_basename(infile_filename);
                if (!outfile_name) {
                    if (CPlusPlus || lang->cplus_runtime_mode()) {
                        Setattr(
                            top, "outfile",
                            NewStringf("%s_wrap.%s", basename, cpp_extension));
                    } else {
                        Setattr(top, "outfile",
                                NewStringf("%s_wrap.c", basename));
                    }
                } else {
                    Setattr(top, "outfile", outfile_name);
                }
                if (!outfile_name_h) {
                    Setattr(top, "outfile_h",
                            NewStringf("%s_wrap.%s", basename, hpp_extension));
                } else {
                ...
                // Check the extension for a c/c++ file.  If so, we're going to
                // declare everything we see as "extern"
                ForceExtern = check_extension(input_file);

                if (tlm->status == Experimental) {
                    Swig_warning(
                        WARN_LANG_EXPERIMENTAL, "SWIG", 1,
                        "Experimental target language. "
                        "Target language %s specified by %s is an experimental "
                        "language. "
                        "Please read about SWIG experimental languages, "
                        "http://swig.org/Doc4.0/"
                        "Introduction.html#Introduction_experimental_status.\n",
    ...
    if (outfiles) {
        ...
        if (!f_outfiles) {
            Printf(stderr,
                   "Failed to write list of output files to the filename '%s' "
                   "specified in CCACHE_OUTFILES environment variable - ",
                   outfiles);
    ...
    if (error_count != 0) SWIG_exit(error_count);
