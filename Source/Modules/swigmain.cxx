...
static TargetLanguageModule modules[] = {
    ...
    {"-emwasm", swig_emwasm, "Emscripten webassembly", Supported},
...
void SWIG_merge_envopt(const char *env, int oargc, char *oargv[], int *nargc,
                       char ***nargv) {
    if (!env) {
        *nargc = oargc;
...
static void insert_option(int *argc, char ***argv, int index, char const *start,
                          char const *end) {
    int new_argc = *argc;
    char **new_argv = *argv;
    ...
    memmove(&new_argv[index + 1], &new_argv[index],
            sizeof(char *) * (new_argc + 1 - index));
    new_argc++;
...
static void merge_options_files(int *argc, char ***argv) {
    ...
    while (i < new_argc) {
        if (new_argv[i] && new_argv[i][0] == '@' &&
            ...
            new_argc--;
            memmove(&new_argv[i], &new_argv[i + 1],
                    sizeof(char *) * (new_argc - i));
            insert = i;
            b = buffer;
            ...
            if (is_target_language_module) {
                ...
                if (language_module->status == Disabled) {
                    if (language_module->help)
                        Printf(stderr,
                               "Target language option %s (%s) is no longer "
                               "supported.\n",
                               language_module->name, language_module->help);
                    else
                        Printf(stderr,
                               "Target language option %s is no longer "
                               "supported.\n",
                ...
                if (strcmp(argv[i], "--help") == 0) strcpy(argv[i], "-help");
                ...
                // Swig_mark_arg not called as the general -help options also
                // need to be displayed later on
            }
        }
    }
